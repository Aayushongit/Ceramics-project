...
for i = 1:size(a_positions, 1)
    draw_tetrahedron_void(a_positions(i,:), a/12, tet_color, 0.25);
end

for i = 1:size(b_positions, 1)
    draw_octahedron_void(b_positions(i,:), a/12, oct_color, 0.18);
end

empty_tet_voids = [3*a/8 a/8 a/8; a/8 3*a/8 a/8; a/8 a/8 3*a/8; 3*a/8 3*a/8 3*a/8];
for i = 1:size(empty_tet_voids, 1)
    draw_tetrahedron_void(empty_tet_voids(i,:), a/14, [0.5 0.5 0.5], 0.1);
end
